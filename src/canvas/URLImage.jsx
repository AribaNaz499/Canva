...
const URLImage = ({ el, isSelected, onSelect, onChange }) => {
  ...
  const handleTransformEnd = () => {
    ...
    if (isCropping) {
      // CROP LOGIC: Image ka container wahi rahega, andar ka content crop hoga
      const newCrop = {
      ...
      onChange({
        ...el,
        crop: newCrop,
        // Position update taake image apni jagah se hiley nahi
        x: node.x(),
        y: node.y(),
      });
    } else {
      // RESIZE LOGIC: Pura box bada/chota hoga
      onChange({
        ...el,
        x: node.x(),
        y: node.y(),
  ...
  return (
    <Group>
      {/* Ghost Image (sirf crop mode mein peeche dikhegi) */}
      {isCropping && (
        <Image
          image={img}
          x={el.x} y={el.y}
      ...
      {isSelected && (
        <Transformer
          ref={trRef}
          keepRatio={!isCropping} // Crop mode mein free-style cutting
